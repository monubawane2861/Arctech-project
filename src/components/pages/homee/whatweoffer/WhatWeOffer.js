...
const WhatWeOffer = () => {
  ...
  const servicesData = [
    ...
    },
    {
      ...
      details: [], // No specific details for this card
    },
  ];
        ...
        {servicesData.map((service, index) => (
          <div key={index} className="flex flex-col items-center h-full">
            <div className="w-full max-w-md mb-6">
              <img
                src={service.image}
                alt={service.altText} // Use altText from data
                className="w-full h-auto object-cover"
              />
            </div>
            <h3 className="text-2xl font-bold text-red-600 text-center">
              {service.title}
              {service.subtitle && <br />} {/* Conditionally render <br> */}
            ...
            {service.additionalDescription && (
              <p className="text-center ">{service.additionalDescription}</p>
            )}
            {service.additionalDescription2 && (
              <p className="text-center ">{service.additionalDescription2}</p>
            )}
            ...
            <button className={`mt-8 ${buttonStyle}`}>Know More</button>
          </div>
        ))}
      </div>
    </div>
